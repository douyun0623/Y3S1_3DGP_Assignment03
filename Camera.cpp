...
void CSpaceShipCamera::Rotate(float x, float y, float z)
{
	if (m_pPlayer && (x != 0.0f))
	{
		...
		결과는 플레이어 위치를 기준(원점)으로 한 카메라의 위치 벡터이다.*/
...
CFirstPersonCamera::CFirstPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		/*1인칭 카메라로 변경하기 이전의 카메라가 스페이스-쉽 카메라이면 카메라의 Up 벡터를 월드좌표의 y-축이 되도록 한다. 
		  ...
		  1인칭 카메라(대부분 사람인 경우)의 로컬 y축 벡터가 월드좌표의 y-축이 되도록 
		  즉, 똑바로 서있는 형태로 설정한다는 의미이다. 
...
CThirdPersonCamera::CThirdPersonCamera(CCamera* pCamera) : CCamera(pCamera)
{
	...
	if (pCamera)
	{
		...
		축 벡터가 월드좌표의 y-축이 되도록 즉, 똑바로 서있는 형태로 설정한다는 의미이다. 
		그리고 로컬 x-축 벡터와 로컬 z-축 벡터의 y-좌표가 0.0f가 되도록 한다. 
